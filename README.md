# CS50x 2026 - Comprehensive Handnotes

[![MIT License](https://img.shields.io/badge/License-MIT-blue.svg)](https://opensource.org/licenses/MIT)
[![GitHub](https://img.shields.io/badge/GitHub-cs50x--2026--handnotes-black?logo=github)](https://github.com/md-mahmudul-hasan-n-86b-git/CS50x-2026-handnotes)

**Comprehensive handwritten notes for CS50x 2026** - Harvard University's introduction to the intellectual Enterprises of Computer Science and the arts of programming.

---

## 📖 Overview

This repository contains my personal handwritten notes for **CS50x 2026**, covering all lectures, chapters, and programming concepts with my own understanding and detailed explanations. These notes are designed to help students grasp the fundamentals of computer science and programming.

### What's Included:
- ✅ Complete lecture notes (Lectures 0-9)
- ✅ Chapter-wise breakdowns
- ✅ Programming concepts explained
- ✅ Code examples and references
- ✅ Key takeaways and summaries

---

## 📁 Repository Structure

```
CS50x-2026-handnotes/
│
├── README.md (this file)
├── LICENSE (MIT License)
│
├── Lecture-0/
│   ├── Chapter-1-Introduction.md
│   ├── Chapter-2-This is CS50.md
│   └── Chapter-3-Artificial Intelligence.md
│   ├── Chapter-4-Computer Science.md
│   ├── Chapter-5-Unary.md
│   └── Chapter-6-Binary
│   └── ... (continue for all chapters)
│
├── Lecture-1/
│   ├── Chapter-1-...
│   ├── Chapter-2-...
│
├── Lecture-2/
│   ├── ...
│
└── ... (continue for all lectures)
```

### How to Use:
1. Navigate to the lecture folder you're interested in
2. Open the chapter markdown file (`.md`)
3. Read the notes and examples
4. Use as a study guide or reference

---

## 🎯 Learning Outcomes

By studying these notes, you'll understand:
- Computational thinking and problem-solving
- Programming fundamentals (C, Python, SQL, JavaScript, CSS, HTML)
- Data structures and algorithms
- Web development basics
- Database concepts
- Cybersecurity principles etc.

---

## 📚 Course Curriculum

| Lecture | Topics |
|---------|--------|
| **Lecture 0** | Scratch |
| **Lecture 1** | C |
| **Lecture 2** | Arrays |
| **Lecture 3** | Algorithms |
| **Lecture 4** | Memory |
| **Lecture 5** | Data Structures |
| **Lecture 6** | Python |
| **Lecture 7** | SQL |
| **Lecture 7 (special)** | Artificial Intelligence |
| **Lecture 8** | HTML, CSS, JavaScript |
| **Lecture 9** | Flask |
| **Lecture 10** | The End |

---

## 🚀 How to Get Started

### 1. Clone or Download
```bash
git clone https://github.com/md-mahmudul-hasan-n-86b-git/CS50x-2026-handnotes.git
```

### 2. Browse Notes
- Visit any lecture folder
- Open the `.md` files in your preferred markdown reader
- GitHub automatically renders markdown files beautifully

### 3. Study Efficiently
- Start with Lecture 0 and progress sequentially
- Use as supplementary material to CS50x videos
- Reference code examples when needed

---

## 📝 Note-Taking Style

These notes follow a consistent format:
- **Key Concepts**: Main ideas and definitions
- **Code Examples**: Practical programming examples
- **Explanations**: Detailed walkthroughs
- **Common Mistakes**: Things to watch out for
- **Summary**: Quick recap of the chapter

---

## 🔗 External Resources

- [Official CS50x Course](https://cs50.harvard.edu/)
- [CS50x YouTube Channel](https://www.youtube.com/c/cs50)
- [CS50 Documentation](https://cs50.readthedocs.io/)

---

## ✨ Features

- 📖 **Comprehensive Coverage**: All lectures and chapters
- 🎓 **Student-Friendly**: Written with students in mind
- 🔍 **Well-Organized**: Easy to navigate and find topics
- 📱 **GitHub-Friendly**: Markdown format for easy reading
- 🔄 **Open Source**: MIT Licensed for sharing and learning

---

## 📊 Statistics

- **Total Lectures**: 10
- **Total Chapters**: 20+
- **Lines of Notes**: 5000+
- **Code Examples**: 100+
- **Last Updated**: May 2026

---

## 💬 How to Use These Notes

1. **As a Study Guide**: Read alongside CS50x lectures
2. **As a Reference**: Look up specific topics and concepts
3. **As Learning Material**: Share with classmates or students
4. **As a Starting Point**: Adapt for your own learning journey

---

## 🤝 Contributing

While this is a personal learning project, if you find errors or have suggestions:
1. Feel free to create an issue
2. Or submit a pull request with improvements

---

## 📄 License

This project is licensed under the **MIT License** - see the [LICENSE](LICENSE) file for details.

This means you can:
- ✅ Use these notes for personal learning
- ✅ Share them with others
- ✅ Modify and adapt them
- ✅ Use them commercially (with attribution)

---

## 👨‍💻 About These Notes

Created by **Md Mahmudul Hasan** as part of pursuing CS50x 2026 from Harvard University.

These notes represent my personal understanding and learning journey through one of the world's most rigorous computer science courses.

---

## 🔔 Updates & Improvements

I continuously review and improve these notes as I progress through CS50x. Subscribe to this repository to stay updated!

**Last Updated**: May 2026

---

## 📞 Contact & Questions

- GitHub: [@md-mahmudul-hasan-n-86b-git](https://github.com/md-mahmudul-hasan-n-86b-git)

---

## ⭐ Show Your Support

If these notes helped you, please:
- ⭐ Star this repository
- 📤 Share with classmates and friends
- 💬 Leave feedback or suggestions

---

**Happy Learning! 🎓**

---

*Last Updated: May 2026*
*CS50x 2026 - Harvard University*

---

## 🎯 Learning Outcomes

By studying these notes, you'll understand:
- Computational thinking and problem-solving
- Programming fundamentals (C, Python, SQL, JavaScript, CSS, HTML)
- Data structures and algorithms
- Web development basics
- Database concepts
- Cybersecurity principles

---

## 📚 Course Curriculum

| Lecture | Topics |
|---------|--------|
| **Lecture 0** | Computational Thinking, Scratch |
| **Lecture 1** | C Basics, Functions, Variables |
| **Lecture 2** | Arrays, Strings, Memory |
| **Lecture 3** | Algorithms, Search, Sort |
| **Lecture 4** | File I/O, Structures, Dynamic Memory |
| **Lecture 5** | Data Structures (Stack, Queue, Linked List) |
| **Lecture 6** | Python Basics |
| **Lecture 7** | SQL and Databases |
| **Lecture 8** | Cybersecurity |
| **Lecture 9** | Web Development (HTML, CSS, JavaScript) |

---

## 🚀 How to Get Started

### 1. Clone or Download
```bash
git clone https://github.com/md-mahmudul-hasan-n-86b-git/CS50x-2026-handnotes.git
```

### 2. Browse Notes
- Visit any lecture folder
- Open the `.md` files in your preferred markdown reader
- GitHub automatically renders markdown files beautifully

### 3. Study Efficiently
- Start with Lecture 0 and progress sequentially
- Use as supplementary material to CS50x videos
- Reference code examples when needed

---

## 📝 Note-Taking Style

These notes follow a consistent format:
- **Key Concepts**: Main ideas and definitions
- **Code Examples**: Practical programming examples
- **Explanations**: Detailed walkthroughs
- **Common Mistakes**: Things to watch out for
- **Summary**: Quick recap of the chapter

---

## 🔗 External Resources

- [Official CS50x Course](https://cs50.harvard.edu/)
- [CS50x YouTube Channel](https://www.youtube.com/c/cs50)
- [CS50 Documentation](https://cs50.readthedocs.io/)

---

## ✨ Features

- 📖 **Comprehensive Coverage**: All lectures and chapters
- 🎓 **Student-Friendly**: Written with students in mind
- 🔍 **Well-Organized**: Easy to navigate and find topics
- 📱 **GitHub-Friendly**: Markdown format for easy reading
- 🔄 **Open Source**: MIT Licensed for sharing and learning

---

## 📊 Statistics

- **Total Lectures**: 10
- **Total Chapters**: 20+
- **Lines of Notes**: 5000+
- **Code Examples**: 100+
- **Last Updated**: May 2026

---

## 💬 How to Use These Notes

1. **As a Study Guide**: Read alongside CS50x lectures
2. **As a Reference**: Look up specific topics and concepts
3. **As Learning Material**: Share with classmates or students
4. **As a Starting Point**: Adapt for your own learning journey

---

## 🤝 Contributing

While this is a personal learning project, if you find errors or have suggestions:
1. Feel free to create an issue
2. Or submit a pull request with improvements

---

## 📄 License

This project is licensed under the **MIT License** - see the [LICENSE](LICENSE) file for details.

This means you can:
- ✅ Use these notes for personal learning
- ✅ Share them with others
- ✅ Modify and adapt them
- ✅ Use them commercially (with attribution)

---

## 👨‍💻 About These Notes

Created by **Md Mahmudul Hasan** as part of pursuing CS50x 2026 from Harvard University.

These notes represent my personal understanding and learning journey through one of the world's most rigorous computer science courses.

---

## 🔔 Updates & Improvements

I continuously review and improve these notes as I progress through CS50x. Subscribe to this repository to stay updated!

**Last Updated**: May 2026

---

## 📞 Contact & Questions

- GitHub: [@md-mahmudul-hasan-n-86b-git](https://github.com/md-mahmudul-hasan-n-86b-git)

---

## ⭐ Show Your Support

If these notes helped you, please:
- ⭐ Star this repository
- 📤 Share with classmates and friends
- 💬 Leave feedback or suggestions

---

**Happy Learning! 🎓**

---

*Last Updated: May 2026*
*CS50x 2026 - Harvard University*
